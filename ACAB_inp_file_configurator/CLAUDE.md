...
- `static/js/sweep_utils.js` — funciones PURAS del barrido paramétrico (mallas `buildBlocks78`/`calcularVectorTiempos` compartidas con el generador temporal, `buildFluxPatches`/`buildMassPatches`/`buildTimePatches`, `parseSweepValues`, `proposeSuffix`). F20 del BACKLOG — `reconstructFasesFromTimes`/`reconstructFasesFromBlocks78`: inverso de `calcularVectorTiempos`/`buildBlocks78`, reconstruye los tramos REALES (rachas de espaciado constante, con tolerancia relativa `TIME_MESH_REL_TOL`) de una fase a partir de la lista plana `blocks78.times` que trae un inp.5 ya parseado, en vez de colapsarla a un tramo por fase (bug histórico: se conservaban los tiempos finales, se perdían los cortes intermedios sin aviso). Camino ÚNICO usado por `_tryPopulateGeneratorFromTimes` (app.js, generador manual, al cargar un fichero) y por `baseFases` (sweep.js, siembra de la tarjeta 1 del barrido temporal) — cero divergencia entre ambos. Una racha de más de 10 puntos se reparte en varios tramos (límite de `calcularVectorTiempos`, nunca se trunca); si el nº de tramos reconstruidos supera `TIME_MESH_MAX_TRAMOS` (malla irregular sin ninguna racha agrupable), lanza en vez de generar una lista de filas inmanejable — el llamador lo convierte en un aviso (`showToast`/`setMsg`), nunca bloquea el resto de la carga. El aviso de "malla colapsada" que U7 mostraba en la tarjeta sembrada del barrido (`sweep.time_seeded_hint`) se retiró: dejó de ser cierto. Test: `tools/test_sweep_utils.js` (reconstrucción pura, con casos anclados a los ficheros reales `examples/Inp5/exp1.inp.5`/`exp2.inp.5`) y `tools/test_time_mesh_reconstruction.py` (pipeline completo cargar→reconstruir→generar vía subprocess `node` + `_write_inp5`: byte-identidad contra un fichero ya en formato F7, y preservación semántica de la malla —con tolerancia— contra los `exp*.inp.5` reales, anteriores a F7 y por tanto no byte-idénticos en el troceado de tarjetas).
...
- `runner.py` — **común de la suite, mantener sincronizado** con la copia de   `COLLAPS_inp_file_configurator/runner.py`: motor de ejecución single/batch (Fase R3 del runbook runner v2). `app.py` expone `/api/run`, `/api/run/config`, `/api/run/status` (enriquecido con `output_exists`/`workdir` para single y `root` para batch, para el botón "Abrir en Fort Analyzer"), `/api/run/cancel` y `/api/run/log?workdir=<carpeta>` (F9c: run.log de una carpeta arbitraria, para pipelines multi-carpeta donde el workdir de nivel job difiere del cwd del paso que falló).
  ...
  F9 del BACKLOG (Fase 3) — el paso `'run'` de un job admite además las claves opcionales `'stdin'`/`'stdout_file'`: con ellas presentes, el stdout del proceso va al fichero indicado en vez de a `run.log` (stderr sigue yendo a `run.log` por separado); necesario para `chains.exe < input_chain.txt > output_chain.txt`, que rompe la convención de "sin argumentos, todo por cwd" (ver `acab_suite/README.md` §"Invocación de los códigos"). Sin esas claves, comportamiento idéntico al de siempre.
  F9c (hotfix) — `status()` en modo batch rastrea el cwd REAL del paso 'run' en curso (`_current_step_cwd`), no solo el workdir de nivel job: un job puede correr sus pasos en carpetas distintas (pipeline de cadenas: ACAB en `iso_<isótopo>/`, CHAINS en `chains_<isótopo>/`). `read_log_tail(path)` expone la lectura del run.log de cualquier carpeta.
...
- `chains_handler.py` + `templates/chains.html` + `static/js/chains.js` — utilidad CHAINS manual (conversión de un fichero CHAINS suelto, herramienta preexistente e independiente de la sección F9 de abajo). `write_chains_inp` se reutiliza tal cual desde `chains_analysis.py` para generar `input_chain.txt` (las etiquetas de campo del fichero generado son solo para lectura humana, chains.exe hace una lectura posicional).
- `chains_inventory.py` (F9 del BACKLOG) — **fragmento sincronizado** (copia de `leer_concentraciones_iniciales`/`nombre_a_zzaaas` de `ACAB_fort_file_analyzer/fort_analyzer.py`, ver CLAUDE.md raíz): inventario isotópico inicial del fort.6 de referencia (`{acab_key: C_i}`) y códec nombre→ZZAAAS. Se duplica en vez de importar entre apps (procesos Flask independientes, mismo criterio que `coll_writer.py`).
- `chains_analysis.py` (F9 del BACKLOG, Fases 2-3) — sección "Análisis de cadenas": genera, a partir de una carpeta de referencia y N isótopos seleccionados, `tape22/`+`tape24/` (patch de Bloque #11: IWP=3/IMTX=1 sobre la composición de referencia SIN modificar) y un `iso_<isótopo>/`+`chains_<isótopo>/` por isótopo (patch monoisotópico de Bloque #5: INUCL=ZZAAAS(i), XCOMP=C_i×1e-24 — INPT=1, verificación de unidades en `tests/fixtures/chains/PROCEDENCIA.md` — y AJUSTE de Block #2 NUCZO de esa zona a 1, invariante del formato no mencionado explícitamente en el runbook pero obligatorio, ver docstring del módulo). Solo soporta referencias de una única zona activa sin alimentación continua (INFD=0); referencias multi-zona quedan fuera de alcance v1 (422 con mensaje claro). Reutiliza `sweep_writer` (`deep_merge`, `_copy_base_folder`, `_base_exclusion_names`, `_rollback`) pero escribe manifest PROPIO `chains_manifest.json` (referencia, IFINAL/PCNT/NMAX, isótopos con C_i/ZZAAAS/XCOMP/carpetas, `excluded_base_files`), NO un `sweep_manifest.json`. `build_chains_pipeline_jobs` (Fase 3) construye los jobs de `runner.start_batch`: `tape22`→`tape24` (runs de ACAB, 1 paso) seguidos de un job por isótopo con 4 pasos (`run` ACAB monoisotópico → `copy` fort.22 → `copy` fort.24 → `run` chains.exe con `stdin`/`stdout_file`). El run de `tape24` (IMTX=1) termina sin fort.6 y es ÉXITO igualmente (el runner evalúa por código de salida, no por fichero de salida). `chains.exe` (constante `CHAINS_EXE_NAME`, convención fija como `collaps.exe`) se copia desde la referencia a cada `chains_<isótopo>/` en la generación SI está presente allí (su ausencia no rompe la generación, solo el pre-check de ejecución). **F9c (hotfix tras la primera ejecución real):** (1) `chains_<isótopo>/` también se siembra con `REACTIONS.dat`/`DECAY.dat` (`CHAINS_SEED_DATA_FILES`) — sin ellos chains.exe aborta al arrancar (unit 122, forrtl end-of-file, confirmado en run.log real); (2) `build_chains_pipeline_jobs` limpia/re-siembra cada `chains_<isótopo>/` antes de lanzar (`_clean_chains_dir_before_run`: elimina restos de un intento fallido — `output_chain.txt`/`fort.23`/`fort.31` — y re-siembra REACTIONS.dat/DECAY.dat sin condiciones, así que una carpeta ya generada ANTES de este hotfix se autorrepara al reejecutar); (3) el sembrado de tape22/tape24/iso_* excluye además fort.17/22/23/24 obsoletos de la referencia (`_CHAINS_BASE_EXCLUDE_EXTRA`, mismo patrón C4 que sweep_writer). Tests: `tools/test_chains_analysis.py` (generación, bytes contra fixtures oro de `tests/fixtures/chains/`, sembrado/exclusión/limpieza F9c) y `tools/test_chains_analysis_run_endpoint.py` (endpoint + pipeline real con ejecutables falsos D1, incluyendo un `chains.exe` falso que lee stdin/escribe stdout Y comprueba REACTIONS.dat/DECAY.dat no vacíos como el binario real, más un `acab.exe` falso que comprueba sus propios DECAY.dat/XSECTION.dat — regresión F9c: carpeta chains sin REACTIONS.dat → sub-paso CHAINS reportado y run.log visible en `chains_<isótopo>/`). Las tablas de resultados (R_i, Y_z_i, diagrama de cadena — Fases 4-5) NO viven aquí: se consultan en la pestaña "Análisis de cadenas" del `ACAB_fort_file_analyzer`, que lee el `chains_manifest.json` escrito por este módulo (`fort_analyzer.calcular_analisis_cadenas`); ver `docs/manual_usuario.md` §10 de este repo y su equivalente en el analyzer.
- `app.py` — endpoints de la sección "Análisis de cadenas": `GET /api/chains-analysis/inventory?reference_folder=` (inventario del fort.6), `POST /api/chains-analysis/preview`, `POST /api/chains-analysis` (genera), `GET /api/chains-analysis/manifest?root=` (vista de solo lectura: `chains_manifest.json` + `chains_batch_results.json` si existe) y `POST /api/chains-analysis/run` (Fase 3: pre-checks de subcarpetas/ficheros/salida previa + `runner.start_batch` con los jobs de `build_chains_pipeline_jobs`; resultados en `chains_batch_results.json`, endpoint DEDICADO, no reutiliza `/api/run/batch` — el manifest no es un `sweep_manifest.json`). UI: pestaña "Análisis de cadenas" en `index.html` + `static/js/chains_sweep.js` (carga de inventario con checkboxes, previsualizar/generar, panel de ejecución en vivo reutilizando el patrón de `sweep.js` — polling de `/api/run/status`, badges de estado —, y carga de un análisis ya generado). **F9c:** la columna "Paso" de la fila de estado traduce `step_index` vía `pipeline_steps` (de `/api/run/status`, `['acab','copy','copy','chains']`) en vez de `step_type` — antes ambigua ("ACAB/CHAINS" para cualquier paso 'run', pues ACAB monoisotópico y CHAINS comparten tipo); columna nueva "Log" con botón "Ver run.log" en filas fallidas/timeout que pide `/api/run/log?workdir=<carpeta del paso>` (la del paso que de verdad falló, no la del workdir del job) y lo vuelca al panel de log. **F9e:** el botón "Abrir en Fort Analyzer" del panel de ejecución usa `?chains_root=<root>` (antes `?folder=`, mismo query param que el runner de simulación única/barrido) — un `chains_manifest.json` no es una carpeta de "Simulaciones" normal para el analyzer, así que necesita su PROPIO parámetro para aterrizar en la pestaña "Análisis de cadenas" (con su `chainsRoot` propio) en vez de la pestaña "Simulaciones"; ver `static/js/app.js` del analyzer, deep link `?chains_root=`. Sin `target="_blank"` (misma pestaña, patrón U1).
...
python tools/test_chains_analysis.py
python tools/test_chains_analysis_run_endpoint.py
python tools/test_time_mesh_reconstruction.py
```
