...
- `chains_analysis.py` (F9 del BACKLOG, Fase 2) — sección "Análisis de cadenas": genera, a partir de una carpeta de referencia y N isótopos seleccionados, `tape22/`+`tape24/` (patch de Bloque #11: IWP=3/IMTX=1 sobre la composición de referencia SIN modificar) y un `iso_<isótopo>/`+`chains_<isótopo>/` por isótopo (patch monoisotópico de Bloque #5: INUCL=ZZAAAS(i), XCOMP=C_i×1e-24 — INPT=1, verificación de unidades en `tests/fixtures/chains/PROCEDENCIA.md` — y AJUSTE de Block #2 NUCZO de esa zona a 1, invariante del formato no mencionado explícitamente en el runbook pero obligatorio, ver docstring del módulo). Solo soporta referencias de una única zona activa sin alimentación continua (INFD=0); referencias multi-zona quedan fuera de alcance v1 (422 con mensaje claro). Reutiliza `sweep_writer` (`deep_merge`, `_copy_base_folder`, `_base_exclusion_names`, `_rollback`) pero escribe manifest PROPIO `chains_manifest.json` (referencia, IFINAL/PCNT/NMAX, isótopos con C_i/ZZAAAS/XCOMP/carpetas, `excluded_base_files`), NO un `sweep_manifest.json`. `chains.exe` (constante `CHAINS_EXE_NAME`, convención fija como `collaps.exe`) se copia desde la referencia a cada `chains_<isótopo>/` en la generación SI está presente allí (su ausencia no rompe la generación, solo el pre-check de ejecución de la Fase 3). Test: `tools/test_chains_analysis.py` (generación, bytes contra fixtures oro de `tests/fixtures/chains/`).
- `app.py` — endpoints de la sección "Análisis de cadenas" (Fase 2): `GET /api/chains-analysis/inventory?reference_folder=` (inventario del fort.6), `POST /api/chains-analysis/preview`, `POST /api/chains-analysis` (genera) y `GET /api/chains-analysis/manifest?root=` (vista de solo lectura: `chains_manifest.json` + `chains_batch_results.json` si existe). UI: pestaña "Análisis de cadenas" en `index.html` + `static/js/chains_sweep.js` (carga de inventario con checkboxes, previsualizar/generar).
