...
- `app.py` también expone los endpoints del runner (Fase R2 del runbook runner v2): `GET/POST /api/run/config`, `POST /api/run`, `GET /api/run/status`, `POST /api/run/cancel`, `GET /api/run/log?workdir=<carpeta>` (F9c, sincronizado desde `ACAB_inp_file_configurator`: run.log de una carpeta arbitraria — útil en pipelines multi-carpeta donde el workdir de nivel job difiere del cwd del paso en curso, caso del INP configurator; esta app no tiene hoy un pipeline así, pero mantiene el endpoint por ser parte de `runner.py`). Panel de ejecución correspondiente en `templates/index.html`.
