"""Tests de los endpoints de ejecución (Fase R2 del runbook runner v2).
...
cubierto por tools/test_runner.py (Fase R1).
"""
...
class RunEndpointsTestCase(unittest.TestCase):
    ...
    def test_config_roundtrip(self):
        ...
        self.assertEqual(cfg['exe_name'], 'collaps.exe')
        self.assertEqual(cfg['output_file'], 'XSECTION.dat')
    ...
    def test_missing_executable(self):
        ...
        self.assertEqual(res.status_code, 422)
        self.assertIn('collaps.exe', res.get_json()['error'])

    def test_missing_required_files(self):
        (self.tmp / 'collaps.exe').write_text('fake', encoding='utf-8')
        res = self.client.post('/api/run', json={
            'workdir': str(self.tmp), 'save_current': False,
        })
        self.assertEqual(res.status_code, 422)
        self.assertIn('COLL.inp', res.get_json()['error'])
        self.assertIn('XSBL.dat', res.get_json()['error'])

    def test_output_exists_requires_overwrite(self):
        (self.tmp / 'collaps.exe').write_text('fake', encoding='utf-8')
        (self.tmp / 'COLL.inp').write_text('x', encoding='utf-8')
        (self.tmp / 'XSBL.dat').write_text('x', encoding='utf-8')
        (self.tmp / 'XSECTION.dat').write_text('old output', encoding='utf-8')
    ...
    def test_happy_path_saves_current_and_starts(self):
        ...
        with patch.object(runner, 'start') as mock_start:
            ...
            self.assertEqual(kwargs['cmd'], [str(self.tmp / 'collaps.exe')])

        self.assertTrue((self.tmp / 'COLL.inp').exists())
