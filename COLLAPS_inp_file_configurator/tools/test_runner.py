"""Tests del runner de ejecución (Fase R1 del runbook runner v2).
...
class TestBatchLogTailFollowsStepCwd(unittest.TestCase):
    """F9c (sincronizado desde ACAB_inp_file_configurator/tools/test_runner.py):
    en un job cuyo workdir de nivel job difiere del cwd del paso en curso,
    status() debe enseñar el log_tail de la carpeta REAL donde corre el
    paso, no el del workdir de nivel job."""
