...
- **Sesión de vaciado del backlog — F20, F16, F17, F18, F19, C6** ✅ (2026-08-10), 5 commits `inp-conf:`/`analyzer:` + 2 commits `suite:`. F20 (inp-conf, ítem principal): reconstrucción de los tramos reales del historial temporal al cargar un `inp.5` (antes colapsaba a un tramo por fase); byte-idéntico verificado vía pipeline automatizado (subprocess `node` + `_write_inp5`, no solo funciones puras). F16 (analyzer): exportación de pureza en un nodo común entre simulaciones (antes cada una en su propio pico). F19 (analyzer, la del convenio de masa atómica): techo sin portador y masa de yodo con masa atómica real, no número másico (+0,072 % en absolutos, fracciones invariantes hasta 0,001 pp, verificado). F17+F18 (analyzer, unidades `cps`/`adimensional` no absolutas en datos de referencia): comparación de FORMA por mínimos cuadrados en escala logarítmica cuando la exportación de desviación absoluta no tiene sentido físico. C6 (analyzer): parsers de irradiación/enfriamiento alineados con `leer_concentraciones_iniciales` para elementos de símbolo de una letra (499→1211 isótopos detectados en ref_sim). **Sin acceso a navegador real (Playwright) en esta sesión**: las verificaciones de UI de punta a punta pedidas (F20: cargar/guardar/recargar en el generador manual y en el barrido; F16/F17/F18/F19: diálogo de importación y exportaciones CSV en la pestaña Informe Isótopo) quedan pendientes de verificación manual — cada commit lo declara explícitamente. Recuento de suite: 1148 tests, 0 fallos (`run_all_tests.ps1`).
- **Controles F21–F22 (declaración de instantes y nodo común)** ✅ (2026-08-10):   exportación de pureza del experimento 2 verificada tras la corrección. Nodo declarado 3,75 h (intersección de las cinco mesetas), meseta 3,50–4,00 h sin desplazar por T_irr, etiqueta «pico» correcta, y cabecera idéntica al cargar las simulaciones en otro orden. La fracción de impureza de v1 coincide ahora con la publicada en el capítulo (1,3096e-5). Antes: nodo 3,50 h con meseta declarada «3,833 a 4,333 h» —instantes inexistentes en la malla— y un valor un 7,5 % distinto del capítulo. Suite: 1246 tests, 0 fallos.